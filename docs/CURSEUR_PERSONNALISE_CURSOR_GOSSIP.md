...
Documentation en phases pour utiliser `public/cursor_gossip.svg` comme curseur de la souris sur l’ensemble du site The Gossip Project.

**Fichier cible :** `public/cursor_gossip.svg` (icône 32×32, type “bulle / gossip”).
...
2. Ouvrir dans le navigateur : `http://localhost:3000/cursor_gossip.svg`.
3. Vérifier que l’image s’affiche (icône bleue type bulle).  
   Les fichiers dans `public/` sont servis à la racine du site, donc l’URL est bien `/cursor_gossip.svg`.

**Si l’image ne s’affiche pas :** vérifier le chemin du fichier (`public/cursor_gossip.svg`) et les permissions.
...
2. Ajouter une règle sur l’élément qui englobe tout le contenu (en général `body`, ou `html` si besoin) :
   - Propriété : `cursor`
   - Valeur : `url('/cursor_gossip.svg') 0 0, auto`
   - Explication :
     - `url('/cursor_gossip.svg')` : image du curseur (chemin racine du site).
...
2. Pour les **liens et boutons cliquables** : soit ne pas surcharger `cursor` (ils hériteront du `body`), soit définir explicitement `cursor: url('/cursor_gossip.svg') 0 0, pointer` pour garder le même visuel tout en sémantique “pointer”.
...
1. La syntaxe CSS est : `cursor: url('/cursor_gossip.svg') <x> <y>, auto`
...
2. **PNG de secours (optionnel) :** créer une version PNG 32×32 de la même icône (`cursor_gossip.png`), la placer dans `public/`, et utiliser une règle du type :  
   `cursor: url('/cursor_gossip.svg') 0 0, url('/cursor_gossip.png') 0 0, auto`  
...
|-------|------------------------------------|
| 1 | Aucun (vérification URL `/cursor_gossip.svg`) |
...
| 5 | Optionnel : ajout d’un PNG dans `public/` et mise à jour de la règle `cursor` |
...
- Fichier du projet : `public/cursor_gossip.svg` (32×32, déjà prêt pour usage comme curseur)
